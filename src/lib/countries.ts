...
/** ISO country names with international dial codes */
...
export const PHONE_DIAL_CODES = Array.from(
  new Map(COUNTRIES.map((c) => [c.dialCode, c.dialCode])).values()
).sort((a, b) => b.length - a.length || a.localeCompare(b));

export function getDialCodeForCountry(countryName: string): string {
  return COUNTRIES.find((c) => c.name === countryName)?.dialCode ?? '+91';
}

export function formatDisplayMobile(dialCode: string, mobile: string): string {
  return `${dialCode} ${mobile.replace(/\D/g, '')}`;
}
